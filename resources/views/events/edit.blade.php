<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <span class="text-gray-900 font-medium">Edit Event: {{ $event->title }}</span>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8">
                <div class="mb-6 pb-4 border-b border-gray-100">
                    <h2 class="text-xl font-bold text-gray-900">Perbarui Data Event</h2>
                    <p class="text-sm text-gray-500 mt-1">Ubah rincian acara, jadwal pelaksanaan, kuota peserta, atau status publikasi.</p>
                </div>

                <form method="POST" action="{{ route('manage.events.update', $event) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm" required>
                                <option value="draft" {{ old('status', $event->status) == 'draft' ? 'selected' : '' }}>Draft (Belum Dipublikasikan)</option>
                                <option value="upcoming" {{ old('status', $event->status) == 'upcoming' ? 'selected' : '' }}>Upcoming (Akan Datang)</option>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm" required>{{ old('description', $event->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="start_date" name="start_date" type="datetime-local" class="mt-1 block w-full" :value="old('start_date', $event->start_date ? $event->start_date->format('Y-m-d\TH:i') : '')" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="end_date" name="end_date" type="datetime-local" class="mt-1 block w-full" :value="old('end_date', $event->end_date ? $event->end_date->format('Y-m-d\TH:i') : '')" required />
